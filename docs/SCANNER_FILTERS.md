...
* **EV Threshold**: Expected Value (EV) must be >= 2.0% (`polyEV >= 0.02`).
...
* **Weak Starter**: Veto if target team is Home, but Home starting pitcher is **Weak** (composite ERA >= 5.00).
* **Weak Starter**: Veto if target team is Away, but Away starting pitcher is **Weak** (composite ERA >= 5.00).
...
* **Rule**: If a starting lineup contains >= 2 players on a 72h offensive heater (elevated hard-hit/barrel rates), the team receives an advantage boost.
* **Weight Factor**: `WEIGHT_HOT_BAT = 1.2` per hot bat (calibrated down from 2.0 to prevent over-inflation of alpha).
* **Mismatch Boost**: Extra bonus is applied if a team has >= 2 hot bats *and* faces a weak opposing starting pitcher.
...
* **Surging Team Momentum**: If a team has won >= 7 of their last 10 games, it receives a **+2.0** form bonus and is flagged with `🔥 Team Momentum`.
* **Cold Streak**: If a team has won <= 3 of their last 10 games, it receives a **-2.0** form penalty and is flagged with `🥶 Cold Streak` (acting as a risk factor).
...
Composite ERA = (Regular ERA * 0.7) + (Spring ERA * 0.3)
* *Spring Guard*: Spring Training ERA is ignored if the sample size is < 10 innings.
* *Recent Performance Priority*: If a pitcher has >= 15 innings in the *current* active season, that current season's ERA overrides the historical blended fallback entirely.
...
* **Integrity Promotion Override**: Elevates a pitcher to "Elite" status if their calculated composite/current ERA is <= **2.80**.
* **Weak Classification**: Pitchers are classified as "Weak" if they belong to `WEAK_PITCHERS_STATIC` or if their calculated composite/current ERA is >= **5.00**.
