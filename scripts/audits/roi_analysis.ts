...
function analyzeROI() {
    ...
    Object.entries(markets).forEach(([name, mRows]) => {
        ...
        if (buys.length > 0 && end) {
            buys.forEach(buy => {
                ...
                // Wait, if it's a redeem for a portion of the market, the payout should be tokens * 1.0.
                // But the CSV Redeem usdcAmount seems to be the TOTAL payout.
    ...
    Object.entries(buckets).forEach(([name, data]) => {
        ...
        console.log(`${name.padEnd(15)} | Win Rate: ${wr.toFixed(1).padStart(5)}% | ROI: ${roi.toFixed(1).padStart(6)}% | Net: $${(data.payout - data.stake).toFixed(2).padStart(8)}`);
    });
}
